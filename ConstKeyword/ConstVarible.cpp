...
// A must be intializd at the time of declaration

const int x = 10;

// Cont can be used with pointers to specify whether the pointers to 
// specify whether the the pointer itself or the data it points to is constant.
...
*ptr2 = 20; // Allowed: Value can ne modified.
...
Function parameter:- Ensures the function does not modify the argumnet.
Member Function:- Can not modify the argumnet.
Return values:- prevents modification of the returned values.
Object:- Can not be modified: only const member function can be modified.
Mutable Data member:- Can be modified even in const objects or functions.
*/
